...
function dl() {
  local __url=$1
  local __dest=$2
  curl --retry 5 --retry-delay 15 -sSL "$__url" -o "$__dest"
}
...
function package() {
  if [[ -n "$(dpkg --get-selections | grep -P '^$1\s')" ]]; then
    ...
    log "Installing $1"
    DEBIAN_FRONTEND=noninteractive apt-get install $1 -y --no-install-recommends
  fi
}
...
function repo_osquery() {
  ...
  DEBIAN_FRONTEND=noninteractive apt-key adv --keyserver keyserver.ubuntu.com --recv-keys 1484120AC4E9F8A1A577AEEE97A80C63C9D8B80B
  DEBIAN_FRONTEND=noninteractive add-apt-repository "deb [arch=amd64] https://pkg.osquery.io/deb deb main"
}
...
function set_motd() {
  ...
  cp "$__path/extra/motd-ctf.sh" /etc/update-motd.d/10-help-text
}
...
function self_signed_cert() {
  ...
  openssl req -nodes -newkey rsa:2048 -keyout "$__devkey" -out "$__csr" -subj "/O=Facebook CTF"
  openssl x509 -req -days 365 -in "$__csr" -signkey "$__devkey" -out "$__devcert"
}
...
EOF
    chmod +x /root/tmp/certbot.sh
  else
    /usr/bin/certbot-auto certonly -n --agree-tos --standalone --standalone-supported-challenges tls-sni-01 -m "$__myemail" -d "$__mydomain"
     ln -s "/etc/letsencrypt/live/$__mydomain/fullchain.pem" "$1" || true
     ln -s "/etc/letsencrypt/live/$__mydomain/privkey.pem" "$2" || true
  fi
}
...
function install_nginx() {
  ...
  openssl dhparam -out "$__dhparam" 2048

  if [[ "$__multiservers" == true ]]; then
      cat "$__path/extra/nginx/nginx.conf" | sed "s|CTFPATH|$__path/src|g" | sed "s|CER_FILE|$__cert|g" | sed "s|KEY_FILE|$__key|g" | sed "s|DHPARAM_FILE|$__dhparam|g" | sed "s|HHVMSERVER|$__hhvmserver|g" | tee /etc/nginx/sites-available/fbctf.conf
  else
      cat "$__path/extra/nginx.conf" | sed "s|CTFPATH|$__path/src|g" | sed "s|CER_FILE|$__cert|g" | sed "s|KEY_FILE|$__key|g" | sed "s|DHPARAM_FILE|$__dhparam|g" | tee /etc/nginx/sites-available/fbctf.conf
  fi
...
function install_hhvm() {
  ...
  DEBIAN_FRONTEND=noninteractive apt-key adv --recv-keys --keyserver hkp://keyserver.ubuntu.com:80 0x5a16e7281be7a449
  DEBIAN_FRONTEND=noninteractive apt-key adv --recv-keys --keyserver hkp://keyserver.ubuntu.com:80 0xB4112585D386EB94

  log "Adding HHVM repo"
  DEBIAN_FRONTEND=noninteractive add-apt-repository "deb http://dl.hhvm.com/ubuntu xenial-lts-3.21 main"
  ...
  if [[ "$__multiservers" == true ]]; then
    cat "$__path/extra/hhvm.conf" | sed "s|CTFPATH|$__path/|g" | sed "s|hhvm.server.ip|;hhvm.server.ip|g" | sed "s|hhvm.server.file_socket|;hhvm.server.file_socket|g" | tee "$__config"
  else
    cat "$__path/extra/hhvm.conf" | sed "s|CTFPATH|$__path/|g" | sed "s|hhvm.server.port|;hhvm.server.port|g" | tee "$__config"
  fi
  ...
  log "PHP Alternaives:"
  /usr/bin/update-alternatives --display php
...
function hhvm_performance() {
  ...
  hhvm-repo-mode enable "$__path"
  chown www-data:www-data "$__repofile"
  service hhvm restart
}
